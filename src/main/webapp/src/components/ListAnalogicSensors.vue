      ...
      <v-data-table :headers="headers" :items="sensors">
        ...
        <template v-slot:top>
          ...
          <v-dialog v-model="dialogEdit" max-width="500">
            <v-card>
              <v-card-title>
                <h4>Mise à jour analogic sensors {{ editedSensors.id }}</h4>
              </v-card-title>
              <v-card-text>
                <v-text-field
...
import ListSensorData from "@/components/ListAnalogicSensorData.vue";
...
export default {
  ...
  computed: {
    sensors1() {
      return this.$store.getters["sensors/getAllAnalogicSensors"];
    },
  },
  methods: {
    deleteSensors(id) {
    ...
    choseSensors(sensors) {
      ...
      this.$store.dispatch("datas/getAnalogicDatasSensors", sensors.id);
    ...
    confirmUpdate() {
      this.$store.dispatch("sensors/updateAnalogicSensors", this.editedSensors);
      this.dialogEdit = false;
    },
    closeSensorSelected() {
